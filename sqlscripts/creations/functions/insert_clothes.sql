...
$BODY$
    BEGIN
        ...
        IF resident IS NOT NULL AND NOT EXISTS( SELECT 1 FROM residents WHERE id = resident ) THEN
            RAISE EXCEPTION "residents( % ) doesn't exist", resident;
            -- TODO: insert non-existing row if it able
        END IF;

        -- check if pattern exists
        IF EXISTS( SELECT 1 FROM patterns WHERE id = pattern ) THEN
            RAISE EXCEPTION "patterns( % ) doesn't exist", pattern;
            -- TODO: insert non-existing row if it able
        END IF;

        -- check if colour exists
        IF EXISTS( SELECT 1 FROM colours WHERE name = colour ) THEN
            RAISE EXCEPTION "colours( % ) doesn't exist", colour;
            -- TODO: insert non-existing row if it able
        END IF;

        -- check if type exists
        IF EXISTS( SELECT 1 FROM types WHERE name = type_nm ) THEN
            RAISE EXCEPTION "types( % ) doesn't exist", type_nm;
            -- TODO: insert non-existing row if it able
        END IF;
